...
--------------------------------------------------------
  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "WS4TI"."PKG_SECURITY" IS
   ...
   FUNCTION authenticate_user(p_email      VARCHAR2
      ...
      v_result     BOOLEAN;
   BEGIN
      -- definir texto padrão do cabeçalho e rodape
      BEGIN
         SELECT 'WS4MIND ' || decode(p_tp_usuario, 'A', '[ADM]', 'T', '[TEC]', '[USER]') || ' | Manutenção Inteligente' INTO v_ds_titulo FROM dual;
      EXCEPTION
         WHEN OTHERS THEN
            v_ds_titulo := 'MIND | Manutenção Inteligente';
      END;
      --v_ds_rodape := 'MIND | Manutenção Industrial Inteligente | WS4TI - Copyright © 2020 | Todos os direitos reservados';
      v_ds_rodape := 'WS4MIND | Manutenção Inteligente - Copyright © 2020 | WS4TI Todos os direitos reservados <br/>...';
      --           
      ...
      IF p_tp_usuario IN ('A', 'T') THEN
         ...
         END IF;*/
         ----
         /*IF (upper(v_tp_usuario) <> upper(p_tp_usuario) AND upper(v_tp_usuario) <> 'A') THEN
         ...
         EXCEPTION
            WHEN OTHERS THEN
               NULL;
         END;*/
      ELSIF p_tp_usuario = 'U' THEN
         v_result := ldap_security_apex_spo(p_email, p_password);
         IF v_result THEN
            v_cd_usuario := p_email;
            v_id_cliente := 1;
         ELSE
            apex_util.set_session_state('LOGIN_MESSAGE', 'Usuário ou senha inválidos!');
            RETURN FALSE;
         END IF;
      END IF;
      ---
